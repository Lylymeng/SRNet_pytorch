...
#from rand_dataloader import psnr
import copy
...
import numpy as np
from MTL_MMAL_config import max_checkpoint_num, proposalN, eval_trainset, set
...
def validation(model, valid_loader, device, criterion, writer, train_step, bpp, epoch):
    ...
    with torch.no_grad():
        for images, labels in valid_loader:
            ...
            images = images.permute(0, 3, 2, 1).to(device)
            labels = labels.view(-1, 1).to(device)
            labels = torch.squeeze(labels)
            cnt += images.shape[0]
            # 模型输出结果
            proposalN_windows_scores, proposalN_windows_logits, proposalN_indices, window_scores, coordinates, raw_logits, aux_logits, local_logits, local_imgs, local_fm, feature_temp = model(
                images, epoch, 'test')
            # 记录特征图与标签
            ...
            # 计算accuracy
            model_label = torch.max(raw_logits, dim=1)[1]
        ...
        print(
            '__________currIter: %d || valid_loss: %.6f || valid_acc: %.6f__________' % (train_step, avg_loss, avg_acc))
    '''
    ...
    n_samples, n_features, h, w = feature_list.shape
    # print("n_samples: ", n_samples)  #[4000]
    # print("n_features: ", n_features)  #[256]
    feature_list_flat = feature_list.reshape(n_samples, n_features * h * w)
    # result_2D = tsne_2D.fit_transform(feature_list)
    result_2D = tsne_2D.fit_transform(feature_list_flat)
    # print('Finished......')
    fig1 = plot_embedding_2D(result_2D, label_list, f'valid_{bpp}bpp_{epoch}epoch')  # 将二维数据用plt绘制出来
    fig1.show()
    plt.close(fig1)
    plt.savefig(f'./MTL_MMAL_output/t-SNE/WOW/valid_{bpp}_epoch{epoch}.jpg')
    # plt.pause(50)
    n_samples, n_features, h, w = feature_list.shape
    #plt.close(fig1)
    print('Finished......')
...
def train(model,
    ...
    for bpp, data_paths in bpp_datasets.items():
        ...
        # 生成tensorboard文件
        writer = SummaryWriter(log_dir=f'./MTL_MMAL_output/tflogs/MTL_MMAL_WOW_{bpp}')
        # 数据处理
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for index, (images, labels) in enumerate(train_loader):
                ...
                images = images.view(-1, 256, 256, 1).to(device)
                images = images.permute(0, 3, 2, 1).to(device)
                # labels = labels.view(16,-1).to(device)    #batch_size=4
                labels = labels.view(16, -1).to(device)
                labels = torch.squeeze(labels)
                ...
                # 任务1索引
                task1_label = torch.max(raw_logits, dim=1)[1]
                # task1_label = torch.max(raw_logits, dim=1).indices
                # print('task1_label',task1_label)
                # 任务2索引
                task2_label = torch.max(aux_logits, dim=1)[1]
                # 任务1准确率计算
                ...
                # 计算损失
                raw_loss = criterion(raw_logits, labels[:, 0])
                local_loss = criterion(local_logits, labels[:, 0])
                label_first_digital = labels[:, 0]
                windowscls_loss = criterion(proposalN_windows_logits,
                                            labels[:, 0].unsqueeze(1).repeat(1, proposalN).view(-1))
                loss1 = raw_loss + local_loss + windowscls_loss
                loss2 = criterion(aux_logits, labels[:, 1])
                ...
                if train_step % valid_interval == 0:
                    ...
                    # 判断收敛
                    if v_acc > 0.80 and len(acc_history) >= 15 and max(acc_history[-15:]) - min(
                            acc_history[-15:]) < 0.01:
                ...
                if train_step % save_interval == 0:
                    torch.save(model.state_dict(), f'./MTL_MMAL_output/Model/WOW/Model_{bpp}_{epoch}epoch.pth')
                    model_save_cnt += 1
        ...
        if best_model_state:
            torch.save(best_model_state,
                       f'./MTL_MMAL_output/Model/WOW/best/bestModel_{bpp}_{epoch}epoch_{highest_accuracy:.4f}.pth')
            best_model_state_path = f'./MTL_MMAL_output/Model/WOW/best/bestModel_{bpp}_{epoch}epoch_{highest_accuracy:.4f}.pth'
...
def test(model, test_loader, device, criterion, weight_path=None):
    ...
    result_2D = tsne_2D.fit_transform(feature_list_flat)
    # print('Finished......')
    fig1 = plot_embedding_2D(result_2D, label_list, f'test_1.0bpp')  # 将二维数据用plt绘制出来
    fig1.show()
    plt.savefig(f'./MTL_MMAL_TSNE/SUNI/test_1.0bpp.jpg')
    # plt.pause(50)
    n_samples, n_features, h, w = feature_list.shape
    plt.close(fig1)
    print('Finished......')
